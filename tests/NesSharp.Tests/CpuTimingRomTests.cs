...
{
    public static TheoryData<string> PassingCpuTimingRoms => new()
    {
        "1-instr_timing.nes",
        "2-branch_timing.nes"
    };
    ...
    [Theory]
    [MemberData(nameof(PassingCpuTimingRoms))]
    public void InstrTimingSinglesPass(string romName)
    {
        var root = FindWorkspaceRoot(AppContext.BaseDirectory);
        var romPath = Path.Combine(root, "test-roms", "nes-test-roms", "instr_timing", "rom_singles", romName);
    ...
    private static string FindWorkspaceRoot(string start)
    {
        ...
        {
            if (Directory.Exists(Path.Combine(directory.FullName, "test-roms", "nes-test-roms", "instr_timing", "rom_singles")))
            {
