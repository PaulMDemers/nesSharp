...
- Pulse, triangle, noise, DMC channel state. Initial pulse, triangle, noise, and DMC register/timer state implemented; DMC sample buffer, output shifter, and delta playback are now wired to mapper-backed CPU reads.
...
- DMC DMA timing/corruption quirks after basic DMC playback. Basic DMC sample playback, first-pass CPU stall timing, CPU-read halt deferral, DMC dummy/get alignment, and OAM DMA get/put alignment are implemented; DMC/OAM overlap, delayed halt scheduling nuance, and read-conflict quirks remain pending.
