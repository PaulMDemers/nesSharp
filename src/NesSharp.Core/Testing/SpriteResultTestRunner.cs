...
public static class SpriteResultTestRunner
{
    private const ushort ResultAddress = 0x00F8;
    ...
    private static bool IsAtExitLoop(NesMachine machine)
    {
        var pc = machine.Cpu.ProgramCounter;
        return machine.CpuBus.ReadRaw(pc) == JmpAbsoluteOpcode && machine.CpuBus.ReadWordRaw((ushort)(pc + 1)) == pc;
    }
}
