...
Use `dotnet run --project src\NesSharp.Cli -- trace-dma <rom.nes>` to inspect OAM/DMC DMA event timing while working on DMA overlap compatibility. Use `dotnet run --project src\NesSharp.Cli -- sprdma-report <rom.nes> --summary` for a compact row/diff table before opening the full trace columns.
